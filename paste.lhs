...
>   simpleHTTP nullConf{ port = 3000 } (pasteApp db)
>   disconnect db
...
(Note:  If you get an error about `UTCTime`, try `cabal install time-1.1.2.4`
and then compile with `ghc --make -package time-1.1.2.4 paste.lhs`.
The error is due to the fact that the `HDBC` library depends on an older
version of `time`.)
